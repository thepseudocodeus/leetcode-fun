...
class AdapterRegistry:
    ...
    def register(self, adapter: AdapterInterface[T, R], priority: int = 0) -> Result[bool, str]:
        """
        ...
        try:
            ...
            if validation_result.is_err():
                return Err(f"Adapter validation failed: {validation_result.unwrap_err()}")
    ...
    def find_adapter(self, input_data: T, platform: Optional[str] = None) -> Maybe[AdapterInterface[T, R]]:
        """
    ...
    def process_with_best_adapter(self, input_data: T, platform: Optional[str] = None) -> Result[R, str]:
        """
...
def register_adapter(adapter: AdapterInterface[T, R], priority: int = 0) -> Result[bool, str]:
